...
select 'despesa',
  ...
  -- lateral com limit 1, e nao um left join direto: 'FAZENDA PALMITO' e
  -- 'FAZENDA PALHADAO' sao nomes parecidos e um LIKE que casasse os dois
  -- duplicaria o titulo inteiro. Assim, no maximo uma fazenda por linha.
  left join lateral (
...
from lancamentos_financeiros where conag_id is not null
union all
-- O teste que vale: o dinheiro nao pode nascer nem sumir no rateio.
...
union all
-- Titulo de R$ 0,00 ou R$ 0,01 nao tem como ser dividido: qualquer parte
-- arredonda para menos de um centavo. Ficam inteiros, com atividade geral.
-- Sao 81 titulos e R$ 0,09 no total - nao movem nenhum resultado.
