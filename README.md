...
- **Status**: Fases 1 (Fundação + Cadastros base), 2 (Estoque), 3 (Abastecimentos) e 4 (Rateio & Indicadores) prontas.
...
- **Próximas fases**: detecção de anomalias e alertas configuráveis, trilha de auditoria consolidada, dashboards e relatórios exportáveis. Fora do escopo do sistema por enquanto (sem dado-fonte disponível): custo por tonelada colhida/produzida — depende do volume de colheita, que só existirá quando os apps de cana e grãos forem criados.
- **Colocar no ar**: crie um projeto novo no [Supabase](https://supabase.com) e rode `combustivel_schema.sql` do começo ao fim (o arquivo tem as migrações das Fases 1, 2 e 3 em sequência — a Fase 4 é só leitura, não precisa de migração nova — com passo a passo comentado). Publique a Edge Function `criar-usuario-combustivel` e preencha a URL/chave do projeto no topo de `AECombustivel.html`.
