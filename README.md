...
- **Reprodução** — cadastro dos lotes de fêmeas em reprodução (lotes com "reprodução" no nome) e registro dos lançamentos de sêmen e outros insumos usados na inseminação, por lote/data/custo. CRUD completo.
- **Financeiro** — custo de ração, pasto e reprodução do dia e do mês, custo por animal (variável e total com custo fixo diluído), custo diário e mensal detalhado por curral, valor total em estoque dos insumos, e cadastro de custos fixos mensais da operação (mão de obra, energia, etc.), com o total mensal (variável + fixo). Inclui um **painel de resultados** com gráfico de rosca interativo mostrando quanto cada categoria (ração, pasto, reprodução, e cada tipo de custo fixo) representa do custo total do mês. Também permite registrar **investimentos** (compra de animais, máquinas, etc.), mantidos separados do custo operacional. CRUD completo dos custos fixos e investimentos.
