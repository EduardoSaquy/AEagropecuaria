// Edge Function: criar-usuario-matriz
//
// Cria uma conta de acesso (login + senha) e o perfil correspondente
// em `profiles`, no projeto Supabase da AE Matriz. Só pode ser chamada
// por quem já está logado como admin — a verificação é feita aqui
// dentro, usando a service_role key, que nunca fica exposta no app
// (client-side).
//
// Chamada esperada (do app, autenticado):
//   POST /functions/v1/criar-usuario-matriz
//   Authorization: Bearer <access_token do admin logado>
//   Body: { usuario: "joao.silva", senha: "******", nome: "João Silva" }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { usuario, senha, nome } = await req.json()
